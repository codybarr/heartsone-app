<template>
	<div class="search">
		...
		<div v-else>
			...
			<table
				v-else
				class="table table-hover table-bordered table-sm table-responsive-md"
			>
				<thead class="thead-dark">
					...
					<th class="text-center">
						<a
							@click.prevent="orderBy('rarity')"
							href="#"
							class="column-header"
						>
							Rarity
							<SortIcon
								:sorted="sortField === 'rarity'"
								:sortAscending="sortAscending"
							>
							</SortIcon>
						</a>
					...
					<th class="text-center">
						<a
							@click.prevent="orderBy('cardClass')"
							href="#"
							class="column-header"
						>
							Class
							<SortIcon
								:sorted="sortField === 'cardClass'"
								:sortAscending="sortAscending"
							>
							</SortIcon>
						</a>
					...
					<th class="text-center">
						<a
							@click.prevent="orderBy('attack')"
							href="#"
							class="column-header"
						>
							Attack
							<SortIcon
								:sorted="sortField === 'attack'"
								:sortAscending="sortAscending"
							>
							</SortIcon>
						</a>
					...
					<th class="text-center">
						<a
							@click.prevent="orderBy('health')"
							href="#"
							class="column-header"
						>
							Health/Durability
							<SortIcon
								:sorted="sortField === 'health'"
								:sortAscending="sortAscending"
							>
							</SortIcon>
						</a>
				...
				<tbody>
					<tr v-for="(card, index) in filteredCards" :key="card.id">
						...
						<td class="text-center">
							{{ card.cost }}
							<span v-if="card.cost >= 0" class="hearthstone-icon icon-mana">
							</span>
						</td>
						<td class="text-center">
...
export default {
	...
	methods: {
		...
		filterCards() {
			let tempFilter = this.cards.filter(card =>
				card.name.toLowerCase().includes(this.query.toLowerCase())
			)

			if (this.checkedCardClasses.length > 0) {
				tempFilter = tempFilter.filter(
					card =>
						this.checkedCardClasses.indexOf(card.cardClass.toLowerCase()) > -1
				)
			}

			if (this.checkedCardTypes.length > 0) {
				tempFilter = tempFilter.filter(
					card => this.checkedCardTypes.indexOf(card.type.toLowerCase()) > -1
				)
			}

			if (this.checkedRarities.length > 0) {
				tempFilter = tempFilter.filter(
					card => this.checkedRarities.indexOf(card.rarity.toLowerCase()) > -1
				)
			}

			if (this.selectedCardSets.length > 0) {
				tempFilter = tempFilter.filter(
					card => this.selectedCardSets.indexOf(card.set) > -1
				)
			}
